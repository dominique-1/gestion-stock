            ...
            <form method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-6">
                ...
                <div>
                    ...
                    <select name="stock_status" class="w-full bg-white/10 backdrop-blur-md border border-white/30 rounded-2xl px-6 py-4 text-white focus:outline-none focus:border-white/60 focus:bg-white/20 transition-all duration-300">
                        ...
                        <option value="optimal" {{ request('stock_status') == 'optimal' ? 'selected' : '' }}>🟢 Stock optimal</option>
                        <option value="overstock" {{ request('stock_status') == 'overstock' ? 'selected' : '' }}>🔵 Surstock</option>
                    </select>
                </div>
                ...
                <div class="relative bg-white/10 backdrop-blur-xl rounded-3xl overflow-hidden border border-white/20 hover:border-white/40 transition-all duration-500 hover:scale-105">
                    ...
                    <div class="h-48 bg-gradient-to-br from-indigo-500/20 to-purple-500/20 relative overflow-hidden">
                        ...
                        <div class="absolute top-4 right-4">
                            ...
                            @elseif($product->current_stock > $product->stock_optimal)
                                <div class="bg-blue-500 text-white px-4 py-2 rounded-full text-xs font-bold">
                                    <i class="fas fa-arrow-up mr-2"></i>Surstock
                                </div>
                            @else
                                <div class="bg-green-500 text-white px-4 py-2 rounded-full text-xs font-bold">
                                    <i class="fas fa-check-circle mr-2"></i>Optimal
                                </div>
                            @endif
                        </div>
                    </div>
                    ...
                    <div class="p-6">
                        ...
                        <p class="text-gray-400 text-sm mb-4 line-clamp-2">{{ Str::limit($product->description, 80) }}</p>
                        @endif
                        ...
                        <div class="grid grid-cols-2 gap-4 mb-6">
                            ...
                            <div class="bg-white/5 rounded-2xl p-3">
                                <p class="text-gray-400 text-xs uppercase tracking-wider mb-1">Valeur</p>
                                <p class="text-2xl font-black text-green-400">{{ number_format($product->price * $product->current_stock, 2) }}€</p>
                            </div>
                        </div>
