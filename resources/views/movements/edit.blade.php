        ...
        <div class="relative z-10 px-8 py-12">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-gray-300 text-lg">Modifiez les informations du mouvement de stock</p>
                </div>
    ...
    <div class="px-8 py-8">
        <div class="max-w-4xl mx-auto">
            ...
            <div class="bg-white/10 backdrop-blur-xl rounded-3xl p-8 border border-white/20">
                <form method="POST" action="{{ route('movements.update', $movement->id) }}" class="space-y-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <select name="product_id" class="w-full bg-white/10 backdrop-blur-md border border-white/30 rounded-2xl px-6 py-4 text-white focus:outline-none focus:border-white/60 focus:bg-white/20 transition-all duration-300" required>
                                ...
                                @foreach($products as $product)
                                    <option value="{{ $product->id }}" {{ $product->id == $movement->product->id ? 'selected' : '' }} class="text-gray-900">{{ $product->name }} (Stock: {{ $product->current_stock ?? 0 }})</option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div>
                            ...
                            <div class="flex space-x-6">
                                <label class="flex items-center bg-white/10 backdrop-blur-md border border-white/30 rounded-2xl px-6 py-4 cursor-pointer hover:bg-white/20 transition-all duration-300">
                                    <input type="radio" name="type" value="in" class="mr-3" {{ $movement->type == 'in' ? 'checked' : '' }} required>
                                    <span class="text-white font-semibold">📥 Entrée</span>
                                </label>
                                <label class="flex items-center bg-white/10 backdrop-blur-md border border-white/30 rounded-2xl px-6 py-4 cursor-pointer hover:bg-white/20 transition-all duration-300">
                                    <input type="radio" name="type" value="out" class="mr-3" {{ $movement->type == 'out' ? 'checked' : '' }} required>
                        ...
                        <div>
                            ...
                            <input type="number" name="quantity" min="1" value="{{ $movement->quantity }}" class="w-full bg-white/10 backdrop-blur-md border border-white/30 rounded-2xl px-6 py-4 text-white placeholder-white/50 focus:outline-none focus:border-white/60 focus:bg-white/20 transition-all duration-300" placeholder="Entrez la quantité" required>
                        </div>

                        <!-- Date -->
                        <div>
                            <label class="text-white/80 text-sm font-bold uppercase tracking-wider mb-2 block">Date du mouvement</label>
                            <input type="datetime-local" name="moved_at" class="w-full bg-white/10 backdrop-blur-md border border-white/30 rounded-2xl px-6 py-4 text-white focus:outline-none focus:border-white/60 focus:bg-white/20 transition-all duration-300" value="{{ $movement->moved_at instanceof \Carbon\Carbon ? $movement->moved_at->format('Y-m-d\TH:i') : $movement->moved_at }}">
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="reason" rows="3" class="w-full bg-white/10 backdrop-blur-md border border-white/30 rounded-2xl px-6 py-4 text-white placeholder-white/50 focus:outline-none focus:border-white/60 focus:bg-white/20 transition-all duration-300" placeholder="Décrivez le motif du mouvement...">{{ $movement->reason ?? '' }}</textarea>
                    </div>

                    <!-- Notes -->
                    <div>
                        <label class="text-white/80 text-sm font-bold uppercase tracking-wider mb-2 block">Notes (optionnel)</label>
                        <textarea name="note" rows="2" class="w-full bg-white/10 backdrop-blur-md border border-white/30 rounded-2xl px-6 py-4 text-white placeholder-white/50 focus:outline-none focus:border-white/60 focus:bg-white/20 transition-all duration-300" placeholder="Informations complémentaires...">{{ $movement->note ?? '' }}</textarea>
                    </div>
                    ...
                    <div class="flex justify-end space-x-4 pt-6">
                        ...
                        <button type="submit" class="bg-gradient-to-r from-green-600 to-emerald-600 text-white px-8 py-4 rounded-2xl font-bold hover:scale-105 transition-all duration-300 shadow-2xl hover:shadow-green-500/50">
                            <i class="fas fa-save mr-3"></i>Mettre à jour le mouvement
                        </button>
                    </div>
                </form>
            </div>
